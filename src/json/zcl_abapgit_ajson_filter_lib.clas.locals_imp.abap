...
CLASS lcl_empty_filter IMPLEMENTATION.
  METHOD zif_abapgit_ajson_filter~keep_node.
    ...
    temp1 = boolc( ( iv_visit = zif_abapgit_ajson_filter=>visit_type-value AND is_node-type <> zif_abapgit_ajson_types=>node_type-number AND is_node-value IS NOT INITIAL ) OR ( iv_visit = zif_abapgit_ajson_filter=>visit_type-value AND is_node-type = zif_abapgit_ajson_types=>node_type-number AND is_node-value <> '0' ) OR ( iv_visit <> zif_abapgit_ajson_filter=>visit_type-value AND is_node-children > 0 ) ).
    rv_keep = temp1. " array & object
